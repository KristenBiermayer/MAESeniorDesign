...
const float MAX_SPEED = 2;                // Maximum operable speed of our robot in inches per sec
const int MAX_TRIES_ULTRASONIC = 5;      // number of times ultrasonic sensor will try to measure distance upon receiving invalid measure
const float ULTRASONIC_DISTANCE_ERROR_CLOSE = 0.8;
const float WALL_MOUNT_DISTANCE = 2.5;
...
//  Serial.println(distance_to_obstacle_left);
  float distance_to_obstacle_right = get_raw_distance(ultrasonic_pin_trig_right, ultrasonic_pin_echo_right);
//  Serial.println(distance_to_obstacle_right);
  float distance = min(distance_to_obstacle_left, distance_to_obstacle_right);
//  Serial.print( "Minimum Distance to Obstacle: ");
//  Serial.println(distance_to_obstacle);
//  Serial.print("End of Get Raw Min Distance: ");
//  Serial.println(distance);
  return distance; 
}
