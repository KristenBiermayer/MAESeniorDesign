...
// declare functions
float get_distance_from_wall() {
  // read from ultrasonic sensor and return distance to the wall      
}

float get_distance_from_floor(){
  // read from the ultrasonic sensor in the back of the robot and return distance to the floor
}
...
void loop() {
  float distance_to_obstacle = get_distance();
  if (!is_on_wall){
    distance_to_obstacle = get_distance_from_wall();
  }

  else {
    distance_to_obstacle = get_distance_from_floor();
  }
