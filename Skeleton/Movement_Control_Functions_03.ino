...
void orient_steering(){
  ...
  int servo_angle = map(joystick_input, 0,1023,0,180);  // the joystick takes analog values from 0 to 1023. writeMicroseconds command takes 1000 to 2000 as input. use the map function to assign our joystick input to our new range
  Serial.print("The Joystick Input is");
  Serial.println(servo_angle);
  servo_left.write(servo_angle);          
  servo_right.write(servo_angle);
  delay(100); 
  servo_left.detach();
  servo_right.detach();  
  delay(15); 
